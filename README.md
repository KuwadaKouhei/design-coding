...
**ドキュメント作成フェーズ（`/1-requirements`・`/2-design`・`/3-claude-md`）のドキュメントは「1フェーズ＝1コミット」** です。
要件定義 / 各設計ステップ（実現可能性 / 思想＋Git運用 / 技術選定 / アーキ設計 / DB設計 / ディレクトリ構造 / タスク分解） /
CLAUDE.md生成で作成・更新した `docs/`（とルートの `CLAUDE.md`）をフェーズ単位でまとめて1コミットにします
（例: 思想フェーズは `philosophy/*` 3ファイルと `GIT_CONVENTIONS.md` を1コミット）。メッセージは `docs:` タイプが既定です。
なお実装フェーズ（`/4`以降）は前述のとおり 1機能=1ブランチ=1PR で進めます。

## 導入方法

このワークフローは Claude Code の**プラグイン**として配布します（`.claude-plugin/plugin.json`）。
`commands/`・`skills/`・`agents/` を**リポジトリ直下**に置く公式準拠（カノニカル）構成で、同梱の
**マーケットプレイス**（`.claude-plugin/marketplace.json`）を git 経由で追加すれば、各プロジェクトへ
ファイルをコピーせずに導入できます。

### 新規プロジェクトへ導入する

```bash
# 1) マーケットプレイスを追加（GitHub の owner/repo 形式で。相対パスは git 経由でのみ有効）
/plugin marketplace add KuwadaKouhei/design-coding

# 2) プラグインをインストール（plugin名@marketplace名）
/plugin install design-coding@kuwada-plugins

# 以降の更新
/plugin marketplace update kuwada-plugins
```

導入後、コマンドは**プラグイン名の名前空間付き**で呼び出します（例: `/design-coding:1-requirements`、
`/design-coding:2-design`、`/design-coding:status`）。番号と実行順はそのまま保たれます。

> プラグイン経由のコマンド/スキル/ペルソナは常に `プラグイン名:名前` の形式になります（公式仕様）。呼び出しを
> 短くしたい場合は `.claude-plugin/plugin.json` の `name` を短い値（例: `dc`）に変えると `/dc:1-requirements`
> になります。

### このリポジトリ自体を開発・お試しする

クローンしたこのリポジトリを**ローカルのマーケットプレイス**として追加すれば、編集中の内容をそのまま試せます。

```bash
/plugin marketplace add /path/to/design-coding   # クローン先の絶対パス
/plugin install design-coding@kuwada-plugins
```

## 提供コマンド一覧

導入後は名前空間付き（`/design-coding:1-requirements` 等）で呼び出します。以下は接頭辞を省略した一覧です。
...
各フェーズには担当ペルソナを `agents/` に定義しています。コマンドはこのペルソナに `Agent` ツールで
...
プラグインのカノニカル構成として、`commands/`・`skills/`・`agents/` を**リポジトリ直下**に置きます。

```
.
├── README.md
├── .gitignore
├── .claude-plugin/                  # プラグイン配布用メタデータ
│   ├── plugin.json                  # プラグイン定義（name=design-coding。コンポーネントは root を自動走査）
│   └── marketplace.json             # マーケットプレイス（/plugin marketplace add で参照）
├── commands/                        # 番号＝実行順（プラグイン導入時は /design-coding:<名前>）
│   ├── 1-requirements.md
│   ├── 2-design.md
│   ├── 2a-feasibility.md
│   ├── 2b-tech-stack.md
