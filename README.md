...
| 2 | 設計（一括） | `/2-design` | feasibility-study → philosophy-definition → technology-selection → design-and-architecture →（必要なら）database-design | feasibility-researcher → software-architect → tech-selector → software-architect → database-designer | 下記すべて |
...
| 4 | 実装 | `/4-implement` | implementation | implementer | コード（CODING_PHILOSOPHY 準拠） |
...
/2-design       # ⓪実現可能性 → ①思想+Git運用 → ②技術選定 → ③アーキ設計 → ④DB設計（必要時）を通しで実行
...
/4-implement    # CLAUDE.md を入口に DESIGN/DATABASE/CODING_PHILOSOPHY/GIT_CONVENTIONS に準拠して実装
/5-test         # TEST_PHILOSOPHY.md に準拠してテストを設計・実装
/6-review       # 4ペルソナでレビュー・監査し docs/REVIEW.md を作成
```
