...
このワークフローを新規プロジェクトで使うには、次の2通りがあります。

### 方法A: プラグインとして導入（推奨・コピー不要）

このリポジトリは Claude Code の**プラグイン**として配布できます（`.claude-plugin/plugin.json`）。
同梱の**マーケットプレイス**（`.claude-plugin/marketplace.json`）を git 経由で追加すれば、
`.claude/` フォルダを各プロジェクトへコピーせずに導入できます。
...
> プラグイン経由のコマンド/スキル/ペルソナは常に `プラグイン名:名前` の形式になります。呼び出しを短くしたい
> 場合は `.claude-plugin/plugin.json` の `name` を短い値（例: `dc`）に変えると `/dc:1-requirements` になります。

### 方法B: このリポジトリ内で直接使う（開発・お試し）

このリポジトリ（または `.claude/` 一式）を対象プロジェクト直下に置いて Claude Code を開くと、
コマンドとスキルが**素の名前**（`/1-requirements` 等）で自動的に認識されます。
...
│   ├── plugin.json                  # プラグイン定義（commands/agents/skills は .claude/ を参照）
